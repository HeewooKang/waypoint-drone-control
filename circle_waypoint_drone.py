...
class CircleWaypointDrone(Node):
    def __init__(self):
        super().__init__('circle_waypoint_drone')
        ...
        self.radius = 4.0
        self.flight_z = 3.0
        self.num_points = 200   # 많을수록 더 둥글게 보임

        self.waypoints = []

        self.waypoints.append((0.0, 0.0, self.flight_z))
    ...
    def control_loop(self):
        ...
        if self.current_wp_index >= len(self.waypoints):
            self.stop_drone()
            self.get_logger().info('Circle flight complete!', throttle_duration_sec=2.0)
            return
